...
(function () {
  ...
  function applyLang(lang) {
    ...
    // 연구 체험 카드 링크: 접근 가능한 이름에 새 탭 안내 포함
    document.querySelectorAll('a.research-card').forEach(function (card) {
      var title = card.querySelector('.research-card-title');
      var btn = card.querySelector('.research-btn [data-i18n]');
      var titleText = title ? title.textContent.trim() : '';
      var btnText = btn ? btn.textContent.trim() : '';
      card.setAttribute('aria-label', (titleText + ' — ' + btnText + ' ' + t('research.openNewTab', lang)).trim());
    });
